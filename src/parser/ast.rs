...
pub fn extract_target_from_vertex(target: Target, vertex: Vertex) -> Vec<Vertex> {
    let mut target_set = HashSet::new();
    target_set.insert(target);

    return walk_vertex_for_targets(&target_set, vertex);
}

pub fn extract_targets_from_vertex(targets: Vec<Target>, vertex: Vertex) -> Vec<Vertex> {
    let mut target_set = HashSet::new();
...
//Extract target ast vertex types from a parent vertex
pub fn walk_vertex_for_targets(targets: &HashSet<Target>, vertex: Vertex) -> Vec<Vertex> {
    let mut matches = vec![];

    if targets.contains(&vertex.as_target()) {
        matches.push(vertex.clone());
    }

    match vertex {
        Vertex::SourceUnit(source_unit) => {
            for source_unit_part in source_unit.0 {
                matches.append(&mut walk_vertex_for_targets(
                    targets,
                    source_unit_part.into(),
                ));
            }
        }
        Vertex::SourceUnitPart(source_unit_part) => match source_unit_part {
            pt::SourceUnitPart::ContractDefinition(box_contract_definition) => {
                ...
                for base in box_contract_definition.base {
                    if base.args.is_some() {
                        ...
                        for arg in args {
                            matches.append(&mut walk_vertex_for_targets(targets, arg.into()));
                        }
                    }
                }
                ...
                for part in box_contract_definition.parts {
                    matches.append(&mut walk_vertex_for_targets(targets, part.into()));
                }
            }
        ...
        Vertex::ContractPart(contract_part) => match contract_part {
        ...
        Vertex::Statement(statement) => match statement {
            pt::Statement::Args(_, named_arguments) => {
                for argument in named_arguments {
                    matches.append(&mut walk_vertex_for_targets(targets, argument.expr.into()));
                }
            }
            ...
            pt::Statement::Revert(_, _, vec_expression) => {
                for expression in vec_expression {
                    matches.append(&mut walk_vertex_for_targets(targets, expression.into()));
                }
            }
            ...
            pt::Statement::Block {
                ...
                for statement in statements {
                    matches.append(&mut walk_vertex_for_targets(targets, statement.into()));
                }
            }
            ...
            pt::Statement::If(_, expression, box_statement, option_box_statement) => {
                ...
                matches.append(&mut walk_vertex_for_targets(targets, box_statement.into()));
        ...
        Vertex::Expression(expression) => match expression {
            pt::Expression::Add(_, box_expression, box_expression_1) => {
                matches.append(&mut walk_vertex_for_targets(targets, box_expression.into()));
            ...
            pt::Expression::List(_, parameter_list) => {
                for (_, option_parameter) in parameter_list {
                    if option_parameter.is_some() {
                        ...
                        matches.append(&mut walk_vertex_for_targets(targets, parameter.ty.into()));
                    }
                }
            }
